...
func NewMemoryFalconDocList() FalconDocList {
	return &MemoryFalconDocList{
		docList: make([]*message.DocId, 0),
		length: 0
	}
}
...
func (mfd *MemoryFalconDocList) FalconDecoding(bytes []byte) error {
	...
	for pos := 0; pos < len(bytes); pos += 8 {
		docid := &message.DocId{
			DocID: binary.LittleEndian.Uint32(bytes[pos: pos + 4]),
			Weight: binary.LittleEndian.Uint32(bytes[pos + 4: pos + 8])
		}
		mfd.docList = append(mfd.docList, docid)
	}
